...
class SnsProxy:
    ...
    __existing_topic_arns: List[str] = None
    ...
    def set_topic_subscriptions_for_owner(self, owner: str, subscriptions: List[SubscriptionConfig]):
        ...
        for existing_subscription in list_subscriptions_response['Subscriptions']:
            if existing_subscription['Endpoint'] not in subscriptions_dict and existing_subscription['SubscriptionArn'] != 'PendingConfirmation':
    ...
    def __get_all_topics(self) -> List[str]:
        if self.__existing_topic_arns is not None and len(self.__existing_topic_arns) > 0 and not self.__existing_topic_arns_is_dirty:
            return self.__existing_topic_arns
        topics = []
        call_list = True
        next_token = ''
        while call_list:
            list_topics_response = self.__sns_client.list_topics(NextToken=next_token)
            ...
            if 'NextToken' in list_topics_response and list_topics_response['NextToken']:
                next_token = list_topics_response['NextToken']
            else:
                call_list = False
    ...
    def __get_topic_arn_for_owner(self, owner: str) -> str:
        return self.__arn_pattern.format(
