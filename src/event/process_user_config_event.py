...
class ProcessUserConfigEvent:
    ...
    def handle(self):
        ...
        if config_object is None:
            failure_message = f'Could not find user config file [{self.__config_name}] in bucket [{self.__config_bucket_name}]'
        ...
        for user_config in config['userConfigs']:
            ...
            self.__sns_proxy.set_topic_subscriptions_for_owner(owner, self.__get_user_subscriptions(user_config['subscriptions']))
    ...
    def __get_user_subscriptions(subs: List[dict]) -> List[SubscriptionConfig]:
        return [SubscriptionConfig(endpoint=sub['endpoint'], protocol_type=ProtocolType(sub['protocol'])) for sub in subs]
